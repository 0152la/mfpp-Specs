...
namespace z3 {
    ...
    class exception;
    class config;
    class __attribute__((annotate("expose"))) context;
    class symbol;
    class params;
    class param_descrs;
    class ast;
    ...
    public:
        ...
        */
        ...
        /**
           ...
           \pre is_numeral()
        */
        unsigned __attribute__((annotate("expose_special"))) get_numeral_uint() const {
            assert(is_numeral());
            unsigned result = 0;
        ...
        */
        ...
        friend __attribute__((annotate("expose"))) expr operator-(expr const & a, int b);
        friend __attribute__((annotate("expose"))) expr operator-(int a, expr const & b);
        friend __attribute__((annotate("expose"))) expr operator-(unsigned a, expr const & b);
        ...
        friend __attribute__((annotate("expose"))) expr operator|(expr const & a, int b);
        friend __attribute__((annotate("expose"))) expr operator|(int a, expr const & b);
        friend __attribute__((annotate("expose"))) expr nand(expr const& a, expr const& b);
        friend __attribute__((annotate("expose"))) expr nor(expr const& a, expr const& b);
        friend __attribute__((annotate("expose"))) expr xnor(expr const& a, expr const& b);

        friend __attribute__((annotate("expose"))) expr min(expr const& a, expr const& b);
        friend __attribute__((annotate("expose"))) expr max(expr const& a, expr const& b);

        expr __attribute__((annotate("expose"))) rotate_left(unsigned i) { Z3_ast r = Z3_mk_rotate_left(ctx(), i, *this); ctx().check_error(); return expr(ctx(), r); }
        expr __attribute__((annotate("expose"))) rotate_right(unsigned i) { Z3_ast r = Z3_mk_rotate_right(ctx(), i, *this); ctx().check_error(); return expr(ctx(), r); }
        expr __attribute__((annotate("expose_special"))) repeat(unsigned i) { Z3_ast r = Z3_mk_repeat(ctx(), i, *this); ctx().check_error(); return expr(ctx(), r); }

        friend expr abs(expr const & a);
        friend expr sqrt(expr const & a, expr const & rm);

        friend expr operator~(expr const & a);
        expr __attribute__((annotate("expose_special"))) extract(unsigned hi, unsigned lo) const { Z3_ast r = Z3_mk_extract(ctx(), hi, lo, *this); ctx().check_error(); return expr(ctx(), r); }
        unsigned __attribute__((annotate("expose_special"))) lo() const { assert (is_app() && Z3_get_decl_num_parameters(ctx(), decl()) == 2); return static_cast<unsigned>(Z3_get_decl_int_parameter(ctx(), decl(), 1)); }
        unsigned __attribute__((annotate("expose_special"))) hi() const { assert (is_app() && Z3_get_decl_num_parameters(ctx(), decl()) == 2); return static_cast<unsigned>(Z3_get_decl_int_parameter(ctx(), decl(), 0)); }
...
#undef _Z3_MK_UN_
    ...
    inline expr operator!=(expr const & a, int b) { assert(a.is_arith() || a.is_bv() || a.is_fpa()); return a != a.ctx().num_val(b, a.get_sort()); }
    inline expr operator!=(expr const & a, unsigned int b) { assert(a.is_arith() || a.is_bv() || a.is_fpa()); return a != a.ctx().num_val(b, a.get_sort()); }
    inline expr operator!=(int a, expr const & b) { assert(b.is_arith() || b.is_bv() || b.is_fpa()); return b.ctx().num_val(a, b.get_sort()) != b; }
    ...
    inline expr operator-(expr const & a, int b) { return a - a.ctx().num_val(b, a.get_sort()); }
    inline expr operator-(int a, expr const & b) { return b.ctx().num_val(a, b.get_sort()) - b; }
    inline expr operator-(unsigned a, expr const & b) { return b.ctx().num_val(a, b.get_sort()) - b; }
    ...
    inline __attribute__((annotate("expose"))) expr operator~(expr const & a) { Z3_ast r = Z3_mk_bvnot(a.ctx(), a); return expr(a.ctx(), r); }
