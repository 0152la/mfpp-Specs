...
namespace checks {
    ...
    void
    check_expr_eq(context_t* ctx, fuzz::FreeVars& fvs, int_term t1, int_term t2)
    {
        yices_reset_context(ctx);
        yices_assert_formula(ctx, yices_eq(t1, t2));
        int_term f_arr[] = { t1, t2 };
    ...
    void
    check_expr_equal_zero_sat(context_t* ctx, fuzz::FreeVars& fvs, int_term t1, int_term t2)
    {
        yices_reset_context(ctx);
        int_term zero = yices_bvconst_zero(BV_SIZE);
